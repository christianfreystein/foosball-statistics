...
class OfflineFoosballTracker:
    def __init__(self, conf_threshold=0.6, max_lost_frames=30, min_init_detections=3, 
        ...
        Args:
            ...
            reid_proximity_thresh (float): Maximum pixel distance for re-identifying a lost track.
    ...
    def process_video(self, all_detections_by_frame):
        ...
        for frame_idx in tqdm(range(num_frames), desc="Processing Frames for Tracking"):
            ...
            # 1. Try to associate detections with active/lost tracks
            matched_detections_indices = set()
            unmatched_detections = []

            # First pass: try to match to active tracks based on prediction or last known position
            for track in self.active_tracks:
                if track.state == 'active':
                    ...
                    for i, det in enumerate(current_frame_detections):
                        ...
                        # A simple distance threshold for association. Tune this.
                        if dist < min_dist and dist < 100: # Example threshold for active tracking
                            min_dist = dist
                            best_match_idx = i
            ...
            # 2. Handle unmatched detections (potential new tracks or re-identification for lost tracks)
            # Create a new list to safely remove items
            remaining_unmatched_detections = []
            for i, det in enumerate(current_frame_detections):
                if i not in matched_detections_indices:
                    remaining_unmatched_detections.append(det)

            # Re-identification for 'lost' tracks from the current set of unmatched detections
            re_identified_in_current_frame = set()
            for track in self.active_tracks:
                if track.state == 'lost':
                    re_identified = False
                    for j, unmatched_det in enumerate(remaining_unmatched_detections):
                        if (unmatched_det, j) in re_identified_in_current_frame: # Already used
                            continue
                        dist = self.euclidean_distance(track.last_known_position, unmatched_det.center)
                        if dist < self.reid_proximity_thresh:
                            track.add_detection(unmatched_det)
                            self.frame_to_tracked_detections_map[frame_idx].append((unmatched_det, track.track_id))
                            re_identified_in_current_frame.add((unmatched_det, j)) # Mark as used
                            re_identified = True
                            break # Found a match, move to next lost track
            
            # Filter out detections that were used for re-identification from remaining_unmatched_detections
            unmatched_detections = [
                det for j, det in enumerate(remaining_unmatched_detections)
                if (det, j) not in re_identified_in_current_frame
            ]

            # 3. Update track states (mark lost, terminate)
            tracks_to_remove = []
            for track in self.active_tracks:
            ...
            # 4. Initiate new tracks from unmatched detections
            ...
            while self.potential_new_tracks and \
                  self.potential_new_tracks[0].frame_idx < frame_idx - (self.min_init_detections + 5):
...
def visualize_foosball_tracking(video_path, detection_json_path, output_video_path, tracker_params, trail_length_frames=60, output_tracked_json_path=None):
    ...
    with tqdm(total=total_frames, desc="Annotating Video") as pbar:
        while True:
            ...
            # --- Draw Track Trails ---
            # Get all tracks (active and terminated) to draw their history
            for track_id, track_obj in tracker.all_tracks_by_id.items():
                # Only draw trails for active or lost tracks that have detections
                if track_obj.detections:
                    trail_points_for_drawing = []
                    ...
                    # We need to filter and add only the relevant ones.
                    for det in track_obj.detections:
                        if det.frame_idx <= current_frame_idx and \
                           (current_frame_idx - det.frame_idx) < trail_length_frames:
                            trail_points_for_drawing.append(det.center)
                        elif det.frame_idx > current_frame_idx:
                            # Break early if we've passed the current frame in stored detections
                            break
                    
                    if len(trail_points_for_drawing) > 1:
                        ...
                        for i in range(1, len(trail_points_for_drawing)):
                            cv2.line(frame, (int(trail_points_for_drawing[i-1][0]), int(trail_points_for_drawing[i-1][1])),
                                     (int(trail_points_for_drawing[i][0]), int(trail_points_for_drawing[i][1])),
                                     trail_color, 8) # Thickness 2 for the trail
...
if __name__ == "__main__":
    ...
    input_video_path = "/home/freystec/test.mp4"
    detection_json_path = "/home/freystec/test.json"
    output_video_path = "/home/freystec/test_tracked.mp4"
    output_tracked_json_path = "/home/freystec/test_tracked_data.json" # Path for JSON output
    # Tracker Initialization Parameters
    tracker_parameters = {
        "conf_threshold": 0.6,
        "max_lost_frames": 30, # Allows track to be lost for ~2 seconds at 30 FPS
        "min_init_detections": 3, # Requires 6 consecutive high-confidence detections to start a track
        "reid_proximity_thresh": 800, # Allows re-identification up to 100 pixels from last known spot
        "process_noise_scale_q": 10.0, # Kalman Filter Q scale: higher for fast changes
