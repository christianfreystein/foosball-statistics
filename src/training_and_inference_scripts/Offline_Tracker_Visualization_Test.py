...
class OfflineFoosballTracker:
    ...
    def process_video(self, all_detections_by_frame):
        ...
        for frame_idx in tqdm(range(num_frames), desc="Processing Frames for Tracking"):
            ...
            # --- Phase 1: Match current detections to ACTIVE tracks (with generous threshold) ---
            # This allows active tracks to pick up distant detections (fast shots)
            for track in self.active_tracks:
                if track.state == 'active':
                    ...
                    for i, det in enumerate(current_frame_detections):
                        ...
                        # Use the aggressive threshold for active track association to allow large jumps
                        if dist < min_dist and dist < self.aggressive_reid_distance_threshold: 
                            min_dist = dist
                            best_match_idx = i
            ...
            # and might reappear in a distant location due to a quick shot.
            matched_unmatched_indices = set() # To track which of the unmatched_detections are used
            ...
            for track in self.active_tracks:
                if track.state == 'lost' and track.frames_since_last_detection <= self.max_frames_for_aggressive_reid:
                    ...
                    for j, unmatched_det in enumerate(unmatched_detections):
                        if j in matched_unmatched_indices: # Detection already matched in this phase
                            continue
...
if __name__ == "__main__":
    ...
    tracker_parameters = {
        ...
        "max_frames_for_aggressive_reid": 15, # Max frames a track can be lost for aggressive re-ID (e.g., 0.5s at 30 FPS)
